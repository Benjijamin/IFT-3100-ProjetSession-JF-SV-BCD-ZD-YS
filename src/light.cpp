...
void Light::setMaterials(Materials* materials) {
	...
	// Créer ou assigner les matériaux nécessaires
	ofMaterial cubeMat;
...
void Light::reset() {
	...
	// Gérer par la classe Materials
	/*
	// configurer le matériau du cube, sera traité par shader
...
void Light::update() {
	...
	if (is_active_light_directional) {
		// transformer la lumière directionnelle avec la souris
	...
	if (is_active_light_point) {
		// transformer la lumière ponctuelle avec la souris
		light_point.setPosition(ofGetMouseX(), ofGetMouseY(), camera_offset * 0.75f);
	}

	if (is_active_light_spot) {
		// transformer la lumière projecteur avec la souris
